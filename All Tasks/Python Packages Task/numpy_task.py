...
# Answer 1
# The above code doesn't create a 2D array because of the way the tuples are 
# passed to the np.array() function.
# When you pass multiple arguments to a function, they are treated as 
# separate arguments, not as a single argument.
# So, in this case, you're passing four separate arguments to the 
# np.array() function: three tuples and a dtype argument.
# To create a 2D array, you need to pass a single argument to the 
# np.array() function.
...
# Shape attribute is (3,), which indicates that there are 
# three elements in the array in one row.
...
# one row (with one element) and three columns.
